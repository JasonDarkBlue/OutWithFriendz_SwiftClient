...
class AddRestaurantViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate{
    
    let TAG = "AddRestaurantViewController"
    ...
    override func viewDidLoad() {
        ...
        /*Displaying previous search cached information*/
        searchTextField.text = lastPlaceSearchQuery as String
        NSLog("cached results: \(lastPlaceSearchResultsArray.count)")
    ...
    func displayLocationInfo(placemark: CLPlacemark){
        ...
        /*Defaulting map to user location*/
    ...
    func mapView(mapView: MKMapView,
        viewForAnnotation annotation: MKAnnotation) -> MKAnnotationView? {
            ...
            if pinView == nil {
                ...
                pinView!.rightCalloutAccessoryView = UIButton(type: UIButtonType.ContactAdd)
            }
            else {
    ...
    func mapView(mapView: MKMapView, annotationView view: MKAnnotationView, calloutAccessoryControlTapped control: UIControl) {
        if control == view.rightCalloutAccessoryView {
            ...
            self.performSegueWithIdentifier("addingRestaurant", sender: self)
        }
    }
    ...
    func calloutTapped(sender:UIGestureRecognizer){
        ...
        self.performSegueWithIdentifier("showGoogleReviewSegue", sender: self)
    }
        ...
        httpGet(request){
            ...
            if error != nil{
                ...
                if(restaurants.count > 0){
                    /*Get non recommended locations from server and match with Places returned from Google*/
                    let recommendationsURL = Config.SERVER_URL+"/restaurants/withinBox?lowerLeftLon=\(southWestCorner.longitude)&lowerLeftLat=\(southWestCorner.latitude)&upperRightLon=\(northEastCorner.longitude)&upperRightLat=\(northEastCorner.latitude)&unratedByUserId=\(self.hostId!)"

                    let requestRecommendation = NSMutableURLRequest(URL: NSURL(string: recommendationsURL)!)
                    self.httpGet(requestRecommendation){
                        (data, error) -> Void in
                        if error != nil{
                            print(error)
                        } else{
                            let recommendationData = data.dataUsingEncoding(NSUTF8StringEncoding)
                            let jsonArray = JSON(data: recommendationData!).arrayValue
                            ...
                            dispatch_async(dispatch_get_main_queue()){
                                ...
                                self.theMapView.showAnnotations(filteredRestaurants, animated: true)
                                self.lastPlaceSearchResultsArray = filteredRestaurants
                            }
                        }
                    }
                }else{
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "showGoogleReviewSegue" {
            /*Send Place_Id*/
    ...
    func convertJsonToArray(restaurantDataString: NSString) -> [Restaurant]{
        ...
        NSLog("\(self.TAG): Google API returned: \(restaurants.count) locations!")
        return restaurants
    }
